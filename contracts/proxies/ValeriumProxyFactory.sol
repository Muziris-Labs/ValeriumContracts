...
contract ValeriumProxyFactory {
     ...
     */
    function deployProxy(address _singleton, bytes memory initializer, bytes32 salt) internal returns (ValeriumProxy proxy) {
        require(isContract(_singleton), "Singleton contract not deployed");

        bytes memory deploymentData = abi.encodePacked(type(ValeriumProxy).creationCode, uint256(uint160(_singleton)));
     ...
     */
    function createProxyWithNonce(address _singleton, bytes memory initializer, uint256 saltNonce) public returns (ValeriumProxy proxy) {
