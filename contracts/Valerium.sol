    ...
    ERC2771Context
{
    ...
    function setupValerium (
        ...
        require(DOMAIN == "0x", "Valerium: already initialized");
     ...
     */
    function executeTx (bytes calldata _proof, address to, uint256 value, bytes calldata data) public payable returns(bool success) {
        // Verifying the proof
     ...
     */
    function executeBatchTx (bytes calldata _proof, address[] calldata to, uint256[] calldata value, bytes[] calldata data) public payable {
        // Verifying the proof
        require(verify(_proof, _useNonce(), TxHash, TxVerifier), "Valerium: invalid proof");
     ...
     */
    function executeRecovery(bytes calldata _proof, bytes32 _newTxHash, address _newTxVerifier, bytes calldata _publicStorage) public payable {
        // Verifying the proof
     ...
     * @param _publicStorage The new public storage
     */
    function ChangeRecovery(bytes calldata _proof, bytes32 _newRecoveryHash, address _newRecoveryVerifier, bytes calldata _publicStorage) public payable {
        // Verifying the proof
        require(verify(_proof, _useNonce(), RecoveryHash, RecoveryVerifier), "Valerium: invalid proof");
